...
7. 根据市场结果计算主动仓位预算。
8. 对主线仓位和主题仓位的 ETF 候选执行前置 ETF 评分/估值门禁。
9. allocator 只在已通过门禁的 ETF 池内分配主线和主题预算；未分配主动预算交给结构守恒模块处理，禁止隐性回补核心 ETF。
...
- 防御仓位：承接主动仓位预算之外的资金；当 ETF 池为空触发 safe mode 时，承接被主动收缩的风险预算。
