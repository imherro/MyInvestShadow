...
1. Tushare：ETF 日线、成交额、基金净值、净值溢价；上证指数背景线使用指数日线。
...
- `GET /api/index`：主页主要内容，包含概览、仓位结构、防御拆层、可选上游状态、净值曲线、`510300.SH`/`510500.SH` 收盘价归一化对比线、目标仓位历史曲线、目标仓位、调仓历史和数据状态。
...
- 目标仓位变化图以时间为 x 轴、每个持仓目标比例为 y 轴，并用 `000001.SH` 上证指数归一化走势作为背景。
