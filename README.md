...
9. allocator 只在已通过门禁的 ETF 池内分配主线和主题预算；过滤掉的主动预算回补核心 ETF 篮子，不再因门禁回到防御仓位。
...
- 防御仓位：承接主动仓位预算之外的资金；ETF 门禁过滤导致的主动预算缺口优先回补核心 ETF 篮子。
...
如果市场结果提供 `sleeve_mix.thematic`，主题仓位不能超过该上限。防御仓位等于 `100% - 实际主动仓位`；门禁过滤造成的主线/主题缺口会回补核心 ETF，不再触发防御仓位回流。
...
如果主题预算小于 5%，最多选择 1 个主题 ETF；如果主题预算大于等于 5%，最多选择 2 个主题 ETF。市场表现不足或门禁不通过的主题预算会回补核心 ETF，不再回到防御仓位。
...
- 门禁过滤掉的主动预算是否回补核心 ETF，而不是隐性回到防御仓位。
